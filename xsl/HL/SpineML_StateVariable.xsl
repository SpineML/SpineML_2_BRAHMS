...
<xsl:template match="SMLCL:StateVariable" mode="assignStateVariable"><xsl:text>
			...
			if (nodeState.hasField("<xsl:value-of select="@name"/>OVER1") &amp;&amp; !finishedThis) 
			{
				...
				if (<xsl:value-of select="@name"/>.size() != numEl) {berr &lt;&lt; "State Variable <xsl:value-of select="@name"/> has a ValueList override but no base Values";}
				for (UINT32 i = 0; i &lt; __tempInputValues__.size(); i += 2) {
					if (__tempInputValues__[i] > numEl-1) {berr &lt;&lt; "State Variable <xsl:value-of select="@name"/> has ValueList index " &lt;&lt; float(i)&lt;&lt; " (value of " &lt;&lt; float(__tempInputValues__[i]) &lt;&lt; ") out of range";}
			...
			if (nodeState.hasField("<xsl:value-of select="@name"/>OVER2") &amp;&amp; !finishedThis) 
			{
				...
				if (<xsl:value-of select="@name"/>.size() != numEl) {berr &lt;&lt; "Experiment State Variable <xsl:value-of select="@name"/> has a ValueList override but no base Values";}
				for (UINT32 i = 0; i &lt; __tempInputValues__.size(); i += 2) {
					if (__tempInputValues__[i] > numEl-1) {berr &lt;&lt; "Experiment State Variable <xsl:value-of select="@name"/> has ValueList index " &lt;&lt; float(i)&lt;&lt; " (value of " &lt;&lt; float(__tempInputValues__[i]) &lt;&lt; ") out of range";}
					<xsl:value-of select="@name"/>[__tempInputValues__[i]] = __tempInputValues__[i+1];
				}
