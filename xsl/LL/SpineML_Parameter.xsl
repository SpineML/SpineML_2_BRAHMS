...
<xsl:template match="SMLCL:Parameter" mode="assignParameter">
			...
			if (nodeState.hasField("<xsl:value-of select="@name"/>OVER1") &amp;&amp; !finishedThis)
			{
				...
				if (<xsl:value-of select="@name"/>.size() != numEl_BRAHMS) {berr &lt;&lt; "Parameter <xsl:value-of select="@name"/> has a ValueList override but no base Values";}
			...
			if (nodeState.hasField("<xsl:value-of select="@name"/>OVER2") &amp;&amp; !finishedThis)
			{
				...
				if (<xsl:value-of select="@name"/>.size() != numEl_BRAHMS) {berr &lt;&lt; "Experiment Parameter <xsl:value-of select="@name"/> has a ValueList override but no base Values";}
				for (UINT32 i_BRAHMS = 0; i_BRAHMS &lt; __tempInputValues__.size(); i_BRAHMS += 2) {
