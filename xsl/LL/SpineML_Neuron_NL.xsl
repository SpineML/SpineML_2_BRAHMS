...
<!-- This xsl file contains a template to output the SystemML Process
     for a neuron body. -->
...
<Process>
	<xsl:comment>Generated by SpineML_Neuron_NL.xsl</xsl:comment>
				...
				<xsl:choose>
					...
					<xsl:otherwise>
						<m><xsl:attribute name="b">
							...
							<xsl:if test="local-name(..)='Configuration' and count(.//SMLNL:Value) &lt; number($curr_pop/SMLLOWNL:Neuron/@size) and not(count(.//@index)=count(.//SMLNL:Value))">
						...
						<xsl:for-each select=".//SMLNL:Value">
							<xsl:if test="local-name(../../..)='Configuration' and count(../SMLNL:Value) &lt; number($curr_pop/SMLLOWNL:Neuron/@size)">
