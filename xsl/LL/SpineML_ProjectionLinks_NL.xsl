		...
		<xsl:for-each select="document(//SMLEXPT:Model/@network_layer_url)">
			...
			<xsl:for-each select="//SMLLOWNL:Projection">
				<xsl:for-each select="./SMLLOWNL:Synapse">
					<!-- This for-each loop loops over the synapses in each projection. The position() counter here gives us the Synapse number -->
					<xsl:variable name="synnum" select="number(position())-1"/>
					<!-- Debug the above with: <xsl:comment>synnum position result:<xsl:value-of select="$synnum"/></xsl:comment> -->
							...
							<xsl:if test="not(../../../SMLLOWNL:Neuron/@url='SpikeSource')">
								...
								<Link>
									...
									<Lag>
										...
										<xsl:choose>
											<xsl:when test="count(../*/SMLNL:Delay/SMLNL:FixedValue)=1 or (count($expt_root//SMLEXPT:ProjectionDelayChange[@src=$source_name and @dst=$target_name and @synapse=$synnum]/SMLNL:Delay/SMLNL:FixedValue)=1 and count(../SMLNL:ConnectionList)=0)">
					...
					</xsl:for-each> <!-- weight update -->
				</xsl:for-each> <!-- synapse -->
			</xsl:for-each> <!-- projection -->
