...
<xsl:template name="alias_replace_helper_2">
	...
	<xsl:choose>
		...
		<xsl:variable name="math" select = "$aliases[1]/SMLCL:MathInline"/>
			...
			<xsl:call-template name="alias_replace_helper_2">
		...
		<xsl:otherwise>
			<xsl:call-template name="alias_replace_2">
				<xsl:with-param name="allaliases" select="$allaliases"/>
...
<xsl:template name="alias_replace_2">
	...
	<xsl:call-template name="add_indices_2">
		<xsl:with-param name="params" select="$params"/>
...
<xsl:template name="add_indices_2_helper_2b">
...
<xsl:template name="add_indices_2_helper_2">
	...
	<xsl:choose>
		...
		<xsl:variable name="endTemp" select="substring-after($end,$param)"/>
			...
			<xsl:choose>
			<xsl:when test="contains('+-*/() =&amp;|,&lt;&gt;',substring($endTemp,1,1))">
				<xsl:call-template name="add_indices_2_helper_2">
					<xsl:with-param name="params" select="$params"/>
					<xsl:with-param name="param" select="$param"/>
...
<xsl:template name="add_indices_2">
...
<xsl:value-of select="$string"/>
		...
		<xsl:otherwise>
		<xsl:if test="local-name($params[1]) = 'Parameter'">
		<xsl:variable name="param" select = "$params[1]/@name"/>
			...
			<xsl:if test="not(local-name($params[1]) ='Parameter')">
		<xsl:variable name="param" select = "$params[1]/@name"/>
			<xsl:choose>
			<xsl:when test="contains($string,$param)">
			<xsl:call-template name="add_indices_2_helper_2">
